...
class lattice :
	...
	def progress(self, T, field=0, exch_energy = J_e, mag_moment = mu_e) :
		...
		flips, dEs, dMs = v_prog(self.lattice_array, self.current_alt_Es, T, field=field, exch_energy=exch_energy, mag_moment=mag_moment)
		...
		#Update all the self.current_alt_Es for the flipped spins or neighbours of them
		for row in range(self.size) :
			for col in range(self.size) :
				...
				#Site or neighbour changed, then update
				if flips[row, col] \
					or flips[prev_row, col] or flips[next_row, col] or flips[row, prev_col] or flips[row, next_col]:
					
					self.current_alt_Es[row, col] = -2 * self.lattice_array[row, col].calculate_energy(field= field, exch_energy = exch_energy, mag_moment = mag_moment)

		#print("After", self.current_alt_Es)
		#print(flips)
		#sys.exit()
