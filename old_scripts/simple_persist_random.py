...
class lattice_site :
	...
	def flip_spin(self, alt_energy, temp, log_rand_num_kbt, field=0, exch_energy = J_e, mag_moment = mu_e) : 
		...
		if alt_energy <= 0 \
			or (alt_energy > 0 and log_rand_num_kbt <  -1. * alt_energy ):
...
class lattice :
	...
	def __init__(self, N, T, field = 0, exch_energy = J_e, mag_moment = mu_e, typ = "r") :
		...
		self.current_alt_energy = np.zeros((N,N))

		#Save the current alternative energies
		for row in range(N) :
			for col in range(N) :
				self.current_alt_energy[row, col] = \
				 -2 * self.lattice_array[row,col].calculate_energy(field= field, exch_energy = exch_energy, mag_moment = mag_moment)

	def iterate_rand_array(self) :
		"""Iterate through a list +1"""

		last = copy.copy(self.rand_array[-1])

		for i in range(len(self.rand_array[:-1])) :
			self.rand_array[i+1] = self.rand_array[i]

		self.rand_array[0] = last
	...
	def progress(self, field=0, exch_energy = J_e, mag_moment = mu_e) :
		...
		for loc in locs :
			...
			flipped, dE, dM = \
				self.lattice_array[row, col].flip_spin(self.current_alt_energy[row,col], self.T, self.rand_array[row*self.size + col], field=field, exch_energy = exch_energy, mag_moment = mag_moment)
			tot += 1	
			...
			if flipped :
				...
				self.current_alt_energy[row, col] = -2 * self.lattice_array[row, col].calculate_energy(field= field, exch_energy = exch_energy, mag_moment = mag_moment)
				self.current_alt_energy[prev_row, col] = -2 * self.lattice_array[prev_row, col].calculate_energy(field= field, exch_energy = exch_energy, mag_moment = mag_moment)
				self.current_alt_energy[next_row, col] = -2 * self.lattice_array[next_row, col].calculate_energy(field= field, exch_energy = exch_energy, mag_moment = mag_moment)
				self.current_alt_energy[row, prev_col] = -2 * self.lattice_array[row, prev_col].calculate_energy(field= field, exch_energy = exch_energy, mag_moment = mag_moment)
				self.current_alt_energy[row, next_col] = -2 * self.lattice_array[row, next_col].calculate_energy(field= field, exch_energy = exch_energy, mag_moment = mag_moment)

			self.net_energy += dE
			self.net_spin += dM

		self.iterate_rand_array()
